...
export class City extends THREE.Group {
  ...
  /**
   * Updates the city state from Solana account data
   * @param {{tiles: number[][]}} cityAccount 
   */
  updateFromSolana(cityAccount) {
    if (!cityAccount) return;

    // Map Solana tile IDs to internal BuildingType
    const typeMap = {
      1: 'road',
      2: 'residential',
      3: 'commercial',
    ...
    for (let x = 0; x < this.size; x++) {
      for (let y = 0; y < this.size; y++) {
        if (!cityAccount.tiles[y] || cityAccount.tiles[y][x] === undefined) continue;

        const tileTypeId = cityAccount.tiles[y][x];
        const tile = this.getTile(x, y);
        if (!tile) continue;

        const currentType = tile.building ? tile.building.type : null;
        const targetType = typeMap[tileTypeId];
        ...
        if (targetType) {
           if (currentType !== targetType) {
             ...
             this.placeBuilding(x, y, targetType);
           }
        } else {
           // Target is empty (0)
           if (tile.building) {
